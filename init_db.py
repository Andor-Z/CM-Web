from app.models import Role, Employee, Department, Cost, Label
from app import db
...
def employee_fake(count=30):
    z = Employee(password='123456',
                 employee_name='总经理',
                 login_name='zjl',
    ...
    for i in range(count + d_count):
        lnum = randint(30, 100)
        full_name = name[i] + name[lnum]
    ...
    for i in range(count, count + d_count):
        login_name = 'bmjl' + str(i + 1 - count)
        e = Employee(password='123456',
                     employee_name=full_names[i],
                     login_name=login_name,
                     member_since=forgery_py.date.date(True),
...
def cost_fake(count=5000):
    ...
    for i in range(count):
        ...
        c = Cost(
            ...
            cost_money=round(uniform(1, 10000), 2),
            dept=e.dept,
            employee=e
        )
        # cost_money 暂时有问题
